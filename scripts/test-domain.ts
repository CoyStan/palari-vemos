...
import type { AppData, ConcreteSlot, Friend, Plan, PlanFriend } from '../src/domain/types';
import { emptyAppData, migrateAndValidate, SCHEMA_VERSION } from '../src/persistence/migrate';
...
assert.equal(
  catchUpStatus(friend({ id: '1', name: 'Ana', lastMetAt: null, createdAt: now.toISOString() }), now),
  'none',
);
assert.equal(
  catchUpStatus(friend({
    id: '1',
    name: 'Ana',
    lastMetAt: null,
...
assert.equal(slotIsBooked(slot89, [{ ...plan710, status: 'cancelled' }]), false);
...
assert.equal(
  calendarDaysBetween(startOfDay(new Date(2026, 2, 7)), startOfDay(new Date(2026, 2, 9))),
  2,
);
assert.equal(daysSince(startOfDay(new Date(2026, 2, 7)).toISOString(), startOfDay(new Date(2026, 2, 9))), 2);

const legacy = JSON.stringify({
  onboardingComplete: true,
  friends: [{ id: 'f1', name: 'Ana', rhythm: 'monthly', createdAt: now.toISOString() }],
  availability: [],
  skipped: [],
  plans: [],
  settings: {},
...
data = completeOnboardingState(emptyAppData(), {
  friend: {
    name: 'Ana',
    photoUri: null,
    phone: '',
...
const withFriend = completeOnboardingState(emptyAppData(), {
  friend: {
    name: 'Ana', photoUri: null, phone: '', shareMethod: 'message',
    rhythm: 'monthly', customDays: 45, lastMetAt: null,
  },
}, (p) => `${p}_x`);
const created = createPlanState(withFriend, slot, [withFriend.friends[0]!.id], {
  title: '', activity: '', place: '', note: '',
}, (p) => `${p}_p`);
assert.equal(created.ok, true);
if (created.ok) {
  const conflict = createPlanState(created.data, slot, [withFriend.friends[0]!.id], {
    title: '', activity: '', place: '', note: '',
  }, (p) => `${p}_p2`);
  assert.equal(conflict.ok, false);
  const done = markPlanDoneState(created.data, created.plan.id, [withFriend.friends[0]!.id]);
  assert.equal(done.plans.find((p) => p.id === created.plan.id)?.status, 'done');
...
assert.ok(!timeline.some((item) => item.type === 'plan' && item.plan.status === 'done'));
