...
type PresetId =
  "weeknights" | "friday" | "saturday" | "sunday" | "custom" | "oneoff";
...
export function AddAvailabilityScreen(_props: Props = {}) {
  ...
  const mode: "create" | "edit" =
    screen === "editAvailability" ? "edit" : "create";
  void _props;
  const today = formatDateKey(new Date());
  const timeFormat24h = data.settings.timeFormat24h;
  const existing =
    mode === "edit"
      ? (data.availability.find((rule) => rule.id === activeAvailabilityId) ??
        null)
      : null;
  ...
  return (
    <Screen contentClassName="gap-6">
      ...
      <Text className="text-body text-muted">
        Pick a preset, then tap the time fields to fine-tune. Changing times
        keeps one-time slots one-time.
      </Text>
              ...
              >
                <Text
                  className={cn(
                    ...
                    recurrence === id ? "text-primary" : "text-ink",
                  )}
                >
                  {lbl}
                </Text>
