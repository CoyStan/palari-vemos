...
import { useApp, type AvailabilityInput, type FriendInput } from '../state/AppProvider';
...
const QUICK_SLOTS: QuickSlot[] = [
  { id: 'weeknights', label: 'Weeknights', hint: 'Mon–Thu, 7–10 PM', kind: 'recurring', daysOfWeek: [1, 2, 3, 4], startMinutes: 19 * 60, endMinutes: 22 * 60 },
  { id: 'friday', label: 'Friday nights', hint: 'Fridays, 7–11 PM', kind: 'recurring', daysOfWeek: [5], startMinutes: 19 * 60, endMinutes: 23 * 60 },
  { id: 'weekend', label: 'Weekend afternoons', hint: 'Sat & Sun, 2–6 PM', kind: 'recurring', daysOfWeek: [0, 6], startMinutes: 14 * 60, endMinutes: 18 * 60 },
  { id: 'oneoff', label: 'One evening this week', hint: 'Tomorrow, 7–9 PM', kind: 'oneoff', daysOfWeek: [], startMinutes: 19 * 60, endMinutes: 21 * 60 },
];

function defaultFriend(name: string, phone = '', photoUri: string | null = null): FriendInput {
  return {
    name,
    photoUri,
    phone,
...
export function OnboardingScreen() {
  ...
  const buildAvailability = (slotId: string | null): AvailabilityInput | null => {
  ...
  if (step === 'friend') {
    return (
      <Screen contentClassName="gap-4">
        ...
        <Text accessibilityRole="header" className="font-sans-bold text-title text-ink">
          Who would you like to see?
        </Text>
        <Text className="text-body text-muted">
          A name is enough. You can add a number or photo later.
        </Text>
        <Button label="Choose one contact" variant="secondary" onPress={() => void onPickContact()} />
        <TextField
          label="Friend’s name"
          value={name}
      ...
      <View className="gap-2">
        {QUICK_SLOTS.map((slot) => {
          ...
          return (
            ...
            >
              <Text className="font-sans-semibold text-body text-ink">{slot.label}</Text>
